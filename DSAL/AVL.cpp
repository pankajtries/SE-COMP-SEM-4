...
public:
    ...
    node* delete_n(node* cur, const string& key1);
    void deleten();

    node* extractmin(node* tree);
    void descending(node* temp);
    void display();
    bool search(node* cur, const string& key1);
    void search_value();
};
...
bool AVL::search(node* cur, const string& key1) {
    if (cur) {
        if (cur->key == key1)
            return true;
        if (cur->key > key1)
...
node* AVL::delete_n(node* cur, const string& key1) {
    if (!cur)
        return cur;
    if (key1 < cur->key)
...
node* AVL::extractmin(node* tree) {
    if (!tree->left)
        return tree->right;
    tree->left = extractmin(tree->left);
    return balance(tree);
}
