...
class quadratic:
    ...
    def __init__(self):
        self.m = int(input("Enter size of Hash Table:"))
        for i in range(self.m):
            self.hasttablenum.append(0)
            self.hastablename.append(0)
    ...
    def initialize(self,arr,names):
        for i in range(len(arr)):
            key=self.hashfunction(arr[i])
            while self.hasttablenum[key]!=0:
                key=(key+(i**2))%self.m
            self.hasttablenum[key]=arr[i]
            self.hastablename[key]=names[i]

    def display(self):
        print("\nKey - Value")
        for i in range(self.m):
            print(i, '-', self.hasttablenum[i], ":", self.hastablename[i])
